...
# --- Ventanas Directivos y Profesores (más grandes y centradas) ---
def abrir_tabla_directivos(titulo):
    ventana = tk.Toplevel()
    ventana.title(titulo)
    ventana.geometry("900x550+300+100")
    ventana.resizable(False, False)
    aplicar_estilos(ventana)
    ...
    for col in columnas:
        tree.heading(col, text=col)
        tree.column(col, width=200 if col == "Gmail" else 150)
...
# -------------------------
# Ayuda (ahora como tabla tipo Treeview)
# -------------------------
def abrir_ayuda():
    ventana = tk.Toplevel()
    ventana.title("Ayuda")
    ...
    columnas = ("Recurso", "URL")
    tree = ttk.Treeview(ventana, columns=columnas, show="headings")
    for col in columnas:
        tree.heading(col, text=col)
        tree.column(col, width=380 if col == "Recurso" else 380)
    tree.pack(fill="both", expand=True, padx=20, pady=10)
    ...
    for texto, url in enlaces:
        tree.insert("", "end", values=(texto, url))

    def abrir_enlace(event):
        item = tree.focus()
        if item:
            _, url = tree.item(item, "values")
            webbrowser.open(url)
